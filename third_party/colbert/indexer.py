...
from third_party.colbert.infra.config import ColBERTConfig, RunConfig
from third_party.colbert.infra.launcher import Launcher

from third_party.colbert.utils.utils import create_directory, print_message

from third_party.colbert.indexing.collection_indexer import encode
...
class Indexer:
    def __init__(self, checkpoint, config=None, verbose: int = 3):
        ...
        if config is None:
            self.checkpoint_config =  AutoConfig.from_pretrained(checkpoint)
        else: 
    ...
    def erase(self, force_silent: bool = False):
        ...
        if len(deleted):
            if not force_silent and not is_debug():
    ...
    def index(self, name, collection, overwrite=False):
        ...
        self.configure(bsize=4, partitions=None)
